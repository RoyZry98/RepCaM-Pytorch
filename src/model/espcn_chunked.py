...
class ESPCN(nn.Module):
    def __init__(self, args):
        ...
        if args.is15s:
            l = 15 * 33 // args.chunk_size
        elif args.is30s:
            l = 3 * 15 * 33 // args.chunk_size
        else:
            l = 9 * 15 * 33 // args.chunk_size
        
        self.prompters = [FixedPatchPrompter_image(args.patch_size // (self.scale)) for _ in range(l)]
    ...
    def forward(self, x, num):
        if self.cafm:
            ...
            #print("Using the prompter")
            idxs = np.clip(num//self.chunk_size, a_min=None , a_max=self.l - 1)
            print(idxs)
            x = self.prompters[idxs](x)
